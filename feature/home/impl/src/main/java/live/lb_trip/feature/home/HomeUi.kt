...
class HomeUi : Ui<HomeState> {
    ...
    override fun Content(state: HomeState, modifier: Modifier) {
        OnboardingScreen(
            onSignupClick = { state.eventSink(HomeEvent.NavigateToSignup) },
            onLoginClick = { state.eventSink(HomeEvent.NavigateToLogin) },
            modifier = modifier,
        )
    }
}
...
private fun OnboardingScreen(
    ...
    Column(modifier = modifier.fillMaxSize()) {
        OnboardingHeroSection(modifier = Modifier.weight(1f))
...
@Composable
private fun OnboardingHeroSection(modifier: Modifier = Modifier) {
    Box(
        modifier = modifier
            .fillMaxWidth()
